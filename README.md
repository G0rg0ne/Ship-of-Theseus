...
- `POST /query` - Run a question through the 4-stage GraphRAG pipeline (intent router → vector search on communities/entities → context pruning → LLM synthesis). Body: `{ "question": string, "mode"?: "auto"|"global"|"local"|"hybrid", "session_id"?: string, "stream"?: boolean }`. **If the user has no Neo4j graph nodes yet** (no documents added to the brain), the endpoint returns immediately with `answer: "Please upload your document first."`, `mode_used: "none"`, and empty `sources` (non-stream JSON or a single SSE `done` event when `stream: true`); the LLM pipeline is not run. With `stream: false` (default): returns JSON `{ "answer", "mode_used", "session_id", "sources": [{ "type", "id", "level"?, "excerpt"?, "label"? }] }`. With `stream: true`: response is `text/event-stream`; each SSE `data:` line is JSON—`{ "content": "..." }` for token chunks, then `{ "done": true, "answer", "mode_used", "session_id", "sources" }`. Conversation history is persisted per session in Redis (TTL 24h); only the last `CHAT_HISTORY_WINDOW` turns are sent to the LLM. Repeated identical questions in the same conversation state may be served from answer cache (cache key includes mode, session, trimmed history fingerprint, and question). Requires auth.
